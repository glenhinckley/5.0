...
{
    class FindTheRegEXPParts : IDisposable
    {
        ...
        public List<varField> FindTheSets(List<string> RuleList, List<varField> _varField)
        {
            ...
            foreach (string l in RuleList)
            {
                ...
                if (SetTest.Contains(".pattern"))
                {

                    string regPatteren = string.Empty;
                    regPatteren = reg.Pattern(l);

                    foreach (varField v in _varField)
                    {
                        if (v.Name == reg.RegexName)
                        {

                            v.Pattern = regPatteren;
                        }
                    }
                ...
                if (SetTest.Contains(".ignorecase"))
                {

                    bool ignorecase = false;
                    ignorecase = reg.IgnoreCase(l);
