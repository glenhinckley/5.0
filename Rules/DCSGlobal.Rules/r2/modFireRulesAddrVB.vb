...
Imports DCSGlobal.Rules.DCSGlobal.Rules
...
Namespace FireRules
    ...
    Public Class modFireRulesAddrVB
        ...
        Public Function Go(ByVal PATIENT_AUDIT_TRAIL_ID As Integer) As Integer    '(ByVal Args() As String)
            ...
            Met.AddMetric("begin rule.go for " + Convert.ToString(PATIENT_AUDIT_TRAIL_ID))

            Dim RuleFailedFlag As Boolean = False
            ...
            Met.AddMetric("initizlian complete for rule.go for " + Convert.ToString(PATIENT_AUDIT_TRAIL_ID))
            ...
            If (_RULE_CONTEXT_ID <> 0) Then
                ...
                _Guid = Met.AddMetricPair("db", "begin getDataByPatientAuditTrailByPatientID")
                ...
                ' sernerio 3  db returned the n flag so were done clean up and get out
                If _RUN_RULES = "N" Then
                    Return 0
                    Exit Function
                End If
                ...
                Met.AddMetric("db", "end getDataByPatientAuditTrailByPatientID", _Guid)
                _Guid = Guid.Empty
                ...
                a = Met.AddMetricPair("codeblock", "begin address")
                ...
                Try
                    ...
                    If (sRunAVSOut = "Y") Then
                        ...
                        _Guid = Met.AddMetricPair("db", "begin getAddresDataTankByHospCode(" + _PAT_HOSP_CODE + ")")

                        dsGetAddressTankData = dss.getAddresDataTankByHospCode(_PAT_HOSP_CODE)  '--[usp_get_tank_address]

                        Met.AddMetric("db", "end getAddresDataTankByHospCode(" + _PAT_HOSP_CODE + ")", _Guid)
                        _Guid = Guid.Empty
                        ...
                        If Not (IsNothing(dsGetAddressTankData)) Then '*******'subba-041110
                            ...
                            Dim AddressGuid As New Guid

                            AddressGuid = Met.AddMetricPair("addrress", "dsGetAddressTankData has begin addres verifictaion ")
                                    ...
                                    If (_strVerifyAddr = "Y" And _AddressRunMode = "Y") Then 'SUBBA-060508
                                        ...
                                        If ((_AddrTypeFilter.Trim() <> "")) And (_AddrTypeFilter.Split(","c).Length > 0) Then
                                            ...
                                            For Each _AddrType As String In _AddrTypeFilter.Split(CChar(",")) 'order G, P,C 'subba-101813
                                                ...
                                                Met.AddMetric("addres verifcation ro addtype" + _AddrType)
                            ...
                            End Try
                            Met.AddMetric("db", "end address verification", AddressGuid)
                        Else

                            Met.AddMetric(" no adress data")
                        End If  '*******'subba-041110
                ...
                Met.AddMetric("codeblock", "end adress", a)
                _Guid = Guid.Empty
                '************************************************************************************************************************************
                '
                        ...
                        If (dsGetAllTankData.Tables(0).Rows.Count = 1) Then
                            ...
                            Else
                                ...
                                For Each dataAuditTrailTable In dsGetAllTankData.Tables 'dsRulesToFireFiltered
                                    ...
                                    Else
                                        ' ***********************************************************************************************
                                        ' this loads the dll assembally
                                        ' ***********************************************************************************************


                                        Dim rr As New List(Of RuleResult)
                                        Dim RuleMessages As New Dictionary(Of Integer, String)
                                        Dim RuleResults As New Dictionary(Of Integer, Integer)
                                        _dll = "C:\usr\Built_DLL\8cb5f09b-2002-4425-8009-947cc8fe0bda.dll"
                                        Dim assembly As Assembly = assembly.LoadFrom(_dll)


                                        '   Dim 
                                        ' string that builds the Method to call
                                        Dim MethodCall As String = String.Empty
                                        Dim type As Type = assembly.GetType("DCSGlobal.Rules.GeneratedDLL.modFireRulesAddrVB")
                                        Dim instance As Object = Activator.CreateInstance(type) '; //creates an instance of that class


                                        Dim RuleMessages_PropertyInfo As PropertyInfo = type.GetProperty("RuleMessages")
                                        Dim RuleResults_PropertyInfo As PropertyInfo = type.GetProperty("RuleResults")
                                        Dim DR_PropertyInfo As PropertyInfo = type.GetProperty("DR")





                                        RuleMessages = CType(RuleMessages_PropertyInfo.GetValue(instance), Global.System.Collections.Generic.Dictionary(Of Integer, String))
                                        ...
                                        For Each dataRulesAuditTrailRow In filteredRows
                                            ...
                                            DR_PropertyInfo.SetValue(instance, dataRulesAuditTrailRow, Nothing)

                                            Dim res As Object



                                            res = type.GetMethod("RunRules").Invoke(instance, New Object() {})



                                            RuleResults = CType(RuleResults_PropertyInfo.GetValue(instance, Nothing), Global.System.Collections.Generic.Dictionary(Of Integer, Integer))

                                            Dim pair As KeyValuePair(Of Integer, Integer)
                                            For Each pair In RuleResults
                                                Dim _rr As New RuleResult()
                                                Dim MsgReturnVal As String = String.Empty

                                                _rr.RuleID = pair.Key
                                                _rr.RuleReturn = pair.Value

                                                RuleMessages.TryGetValue(pair.Key, MsgReturnVal)
                                            ...
                                            Try
                                                ...
                                                If (RULE_COMMAND <> "" And RULE_COMMAND.IndexOf("##") < 0) Then
                                                    ...
                                                    '  Dim res As Object = type.GetMethod("RunRules").Invoke(instance, New Object() {5, 3})

                                                    '' // MethodInfo[] methods = type.GetMethods();
                                                    ' //object res = methods[8].Invoke(instance, new object[] { 5, 3 });
            ...
            ' Met.AddMetric("End rule.go for " + Convert.ToString(PATIENT_AUDIT_TRAIL_ID))

            'If _Metrics = 2 Then
            '    Met.FlushMetricStringToDisc(_LogFilePath + Convert.ToString(PATIENT_AUDIT_TRAIL_ID) + ".log")
            'End If
        ...
        Public Function ValidateAddressBySwappingAddressFields(ByVal iAttempt As Integer, ByVal sPatHospCode As String, ByVal sAddr1 As String, ByVal sAddr2 As String, _
            ...
            Dim sAddrXmlIn As String = String.Empty
            Dim ts As TimeSpan
            Dim sOrigAddr1 As String = sAddr1
            Dim sOrigAddr2 As String = sAddr2
            ...
            Try
                ...
                If (addresses Is Nothing) Then   'subba-022510 commented 'just write log  'subba-042911 'commented
                    ...
                    Throw New NullReferenceException("DCSAddressValidateDPV Server Exception-TrySec") 'SUBBA-062312
                    'Throw New NullReferenceException("DCSAddressValidateDPV Server Exception")
                ...
                Dim sREL As String = String.Empty
                Dim sAQF As String = String.Empty
                Dim sCRT As String = String.Empty
                Dim sDPV As String = String.Empty
                Dim sCNTY As String = String.Empty
                Dim cszCity As String = String.Empty
                Dim cszState As String = String.Empty
                Dim cszZip As String = String.Empty
                '******************************************************************************************************************
        ...
        Private Function InsertBlankRow(ByVal PID As Integer, ByVal PatHospCode As String) As Integer
            Dim r = -1
