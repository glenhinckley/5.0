...
{
    class TennesseeMedicaid : IDisposable
    {
        ...
        private string _VendorName = "BCTN";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("BCTN   sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("BCTN   Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("BCTN Response ErrorCode ", Err);
                            }
                        }
                        ...
                        if (payLoad != null)
                        {
                            ...
                            if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("BCTN Response Payload  ", _RES);
                            }
                        }
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("BCTN Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("BCTN GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
