...
{
    class Wellmark : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "WELLMARK";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("Wellmark sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("Wellmark Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("Wellmark Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("Wellmark GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = soap + Convert.ToString("<s:Envelope xmlns:a='http://www.w3.org/2005/08/addressing' xmlns:s='http://www.w3.org/2003/05/soap-envelope' xmlns:u='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd'>");
            soap = soap + Convert.ToString("<s:Header>");
            soap = soap + Convert.ToString("<a:Action s:mustUnderstand='1'>RealTimeTransaction</a:Action>");
            soap = soap + Convert.ToString("<a:ReplyTo>");
            soap = soap + Convert.ToString("<a:Address>http://www.w3.org/2005/08/addressing/anonymous</a:Address>");
            soap = soap + Convert.ToString("</a:ReplyTo>");
            soap = soap + Convert.ToString("<a:To s:mustUnderstand='1'>https://services.wellmark.com/WellmarkEDIServiceExternal/WellmarkEDIOperations.svc</a:To>");
            soap = soap + Convert.ToString("<o:Security s:mustUnderstand='1' xmlns:o='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd'>");
            soap = soap + Convert.ToString("<o:UsernameToken>");
            soap = soap + Convert.ToString("<o:Username>" + _UserName  + "</o:Username>");
            soap = soap + Convert.ToString("<o:Password Type='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText'>" +   _Passwd    + "</o:Password>");
            soap = soap + Convert.ToString("</o:UsernameToken>");
            soap = soap + Convert.ToString("</o:Security>");
            soap = soap + Convert.ToString("</s:Header>");
            soap = soap + Convert.ToString("<s:Body xmlns:xsd='http://www.w3.org/2001/XMLSchema' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'>");
            soap = soap + Convert.ToString("<COREEnvelopeRealTimeRequest xmlns='http://www.caqh.org/SOAP/WSDL/CORERule2.2.0.xsd'>");
            soap = soap + Convert.ToString("<PayloadType xmlns=''>X12_270_Request_005010X279A1</PayloadType>");
            soap = soap + Convert.ToString("<ProcessingMode xmlns=''>RealTime</ProcessingMode>");
            soap = soap + Convert.ToString("<PayloadID xmlns=''>" + _PayloadID + "</PayloadID>");
           // soap = soap + Convert.ToString("<TimeStamp xmlns=''>06/16/2017 15:43:10 AM</TimeStamp>");
            soap = soap + Convert.ToString("<TimeStamp xmlns=''>" + DateTime.UtcNow.ToString("yyyy-MM-ddThh:mm:fffZ") + "</TimeStamp>");
            
            soap = soap + Convert.ToString("<SenderID xmlns=''>"  + _SenderID  + " </SenderID>");
            soap = soap + Convert.ToString("<ReceiverID xmlns=''>" +   _ReceiverID    + "</ReceiverID>");
            soap = soap + Convert.ToString("<CORERuleVersion xmlns=''>2.2.0</CORERuleVersion>");
            soap = soap + Convert.ToString("<Payload xmlns=''>" + edi + "</Payload>");
            soap = soap + Convert.ToString("</COREEnvelopeRealTimeRequest>");
            soap = soap + Convert.ToString("</s:Body>");
            soap = soap + Convert.ToString("</s:Envelope>");
