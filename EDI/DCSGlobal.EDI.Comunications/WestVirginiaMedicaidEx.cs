...
{
    class WestVirginiaMedicaidEx : IDisposable
    {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(httpData))
                {
                    try
                    {
                        ...
                        var oPayloadID = xd.DocumentElement.SelectSingleNode("PayloadID");
                        if (oPayloadID != null)
                        {
                            PayloadIDRX = oPayloadID.InnerText;
                        }

                        var oPayloadType = xd.DocumentElement.SelectSingleNode("PayloadType");
                        if (oPayloadType != null)
                        {
                            sPayloadType = oPayloadType.InnerText;
                        }
                ...
                {
                    r = -1;
                    log.ExceptionDetails("WVMCD Reading httpData failed ", httpData);
                }
            }
