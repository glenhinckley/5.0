...
{
    class BCWestVerginia : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "BCWV";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("BCWV sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("BCWV Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("BCWV Response ErrorCode ", Err);
                            }
                        }
                        ...
                        { 
                            ...
                            _RES = xd.DocumentElement.GetElementsByTagName("Payload")[0].InnerText;
                             ...
                             if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("BCWV Response Payload  ", _RES);
                            }
                        }
                        ...
                        /*
                         ...
                         this is what needs to be fixed from here
                         */
                        ...
                        /*
                            ...
                            this is what needs to be fixed to here
                        */
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("BCWV Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("BCWV GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = soap + Convert.ToString("<wss:Security xmlns:wss='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd'>");
            soap = soap + Convert.ToString("<wss:UsernameToken>");
            soap = soap + Convert.ToString("<wss:Username>" +  _UserName  + "</wss:Username>");
            soap = soap + Convert.ToString("<wss:Password>" +  _Passwd  + "</wss:Password>");
            soap = soap + Convert.ToString("</wss:UsernameToken>");
            soap = soap + Convert.ToString("</wss:Security>");
            soap = soap + Convert.ToString("</soap:Header>");
            ...
            soap = soap + Convert.ToString("<TimeStamp>" + DateTime.UtcNow.ToString("yyyy-MM-ddThh:mm:ssZ") + "</TimeStamp>");
        ...
        public static X509Certificate2 SelectCertificate()
        {
            ...
            try
            {
                ...
                {
                    if(oCert.Subject.Contains("services.highmark.com"))
                    {
                        return oCert;
                    }
                }
