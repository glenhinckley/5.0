...
{
    class Centene_v2 : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "CENTENE";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("CENTENE sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("CENTENE Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("CENTENE Response ErrorCode ", Err);
                            }
                        }
                        ...
                        if (payLoad != null)
                        {
                            ...
                            if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("CENTENE Response Payload  ", _RES);
                            }
                        }
                        ...
                        using (VALIDATE_REQ_RES_PAIR v = new VALIDATE_REQ_RES_PAIR())
                        {
                           
                            v.ConnectionString = _ConnectionString;
                            r = v.WhatTheHellAmI(fctREQ.Text, fctRES.Text);
                            //lblValidateRES.Text = Convert.ToString(r) + "  " + v.ErrorMessage;
                        }

                        // IF R = POSTINVE GOT VENDOPR AGAIN
                        {
                        ...
                        // IF R  = 0 DONE
                        // IF R NEGATIVE HARD FAILURE WERE DONE
                        {
                            MLOG_ID = log.MLOG(_VendorName, Err, _EBRID, stime, rtime, B_RES, B_REQ, PayloadIDTX, PayloadIDRX, sPayload, _REQ);
                        }
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("CENTENE Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
