...
{
    class MontanaBatchRetrievalTransaction : IDisposable
    {
        ...
        private string _VendorName = "MONTRET";
        ...
        ~MontanaBatchRetrievalTransaction()
        {
            Dispose(false);
        }
        public void Dispose()
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("MontanaBatchRetrievalTransaction sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                if (_VMetricsLogging == 1)
                {
                    log.ExceptionDetails("MontanaBatchRetrievalTransaction Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("MontanaBatchRetrievalTransaction Response ErrorCode ", Err);
                            }
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("MontanaBatchRetrievalTransaction Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("MontanaBatchRetrievalTransaction GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
       ...
       private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = soap + Convert.ToString("<PayloadType>X12_005010_Request_Batch_Results_271</PayloadType>");
            soap = soap + Convert.ToString("<ProcessingMode>Batch</ProcessingMode>");
            soap = soap + Convert.ToString("<PayloadID>69d810cb-bb06-4e84-aef3-35a39fd8bc37</PayloadID>");
            ...
            soap = soap + Convert.ToString("<SenderID>100000</SenderID>");
            soap = soap + Convert.ToString("<ReceiverID>7230865</ReceiverID>");
            soap = soap + Convert.ToString("<CORERuleVersion>2.2.0</CORERuleVersion>");
            soap = soap + Convert.ToString("<CheckSum>" + sChkSum + "</CheckSum>");
            soap = soap + Convert.ToString("<Payload></Payload>");
            soap = soap + Convert.ToString("</cor:COREEnvelopeBatchSubmission>");
            soap = soap + Convert.ToString("</soap:Body>");
        ...
        private HttpWebRequest CreateWebRequest(string url)
        {
            ...
            webRequest.ContentType = "application/soap+xml;charset=UTF-8;action=BatchResultsRetrievalTransaction";
