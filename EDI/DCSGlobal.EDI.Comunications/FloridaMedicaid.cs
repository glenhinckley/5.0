...
{
    class FloridaMedicaid : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "FLMCD";
        ...
        private int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("FMCD sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("FMCD Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("FMCD Response ErrorCode ", Err);
                            }
                        }
                        ...
                        { 
                            ...
                            _RES = _RES.Replace("<![CDATA[", "").Replace("]]>", "");
                             sPayload = _RES;
                             r = 0;
                             if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("FMCD Response Payload  ", _RES);
                            }
                        }
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("FMCD Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("FMCD GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " | EBR ID:" + _EBRID  + " | Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
