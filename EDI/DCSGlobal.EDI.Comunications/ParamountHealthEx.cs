...
{
    class ParamountHealthEx : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "PARAMOUNTEDI";
        ...
        public int GetResponse()
        {
            ...
            while (i < _VendorRetrys)
            {
                ...
                else
                {
                    ...
                    log.ExceptionDetails(_VendorName, _EBRID + " retry i=" + Convert.ToString(i) + " Error Message " + ErrorMsg);
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                            {
                                ...
                                {
                                    log.ExceptionDetails("ParamountHealthEx Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                                }
                            }
                        }
                        catch (Exception e)
                        {
                            r = -101;
                            log.ExceptionDetails(_EBRID + "- webRequest Error - ParamountHealthEx", _EBRID + " webRequest Error");
                        }
                    ...
                    {
                        r = -102;
                        log.ExceptionDetails(_EBRID + "- IAsyncResult Error - ParamountHealthEx", _EBRID + " IAsyncResult Error");
                    }
                ...
                {
                    r = -103;
                    log.ExceptionDetails(_EBRID + "-InsertSoapEnvelopeIntoWebRequest Error - ParamountHealthEx", _EBRID + " InsertSoapEnvelopeIntoWebRequest Error");
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("ParamountHealthEx Response ErrorCode ", Err);
                            }
                        }
                        ...
                        if (payLoad != null)
                        {
                            ...
                            if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("ParamountHealthEx Response Payload  ", _RES);
                            }
                        }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        log.ExceptionDetails("ParamountHealthEx Reading Response Failed ", ex);
                    }
                }
                else
                {
                    r = -104;
                    log.ExceptionDetails("ParamountHealthEx GetResponse Failed With SoapRespone NULL for the ebr id" + _EBRID  + _VendorName + "| Payor Code: " + _PayorCode + "|" + _wsUrl + "|" + _REQ, soapResult.ToString());
                }
            ...
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("ParamountHealthEx GetResponse Failed with EBR ID:" +  _EBRID     +   " | Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
        ...
        private HttpWebRequest CreateWebRequest(string url)
        {
            ...
            webRequest.ContentType = "text/xml;charset=utf-8"; // "application/soap+xml;charset=UTF-8;action=RealTimeTransaction";    // "multipart/related";  //"text/xml;charset='utf-8'";
            webRequest.Accept = "text/xml";  // "application/xop+xm"; // "application/soap+xml"; // "text/xml"; //"application/soap+xml";
            webRequest.Method = "POST";
            webRequest.Timeout = Convert.ToInt32(60000);
            return webRequest;
        }
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = soap + "<PayloadID>" + sPayloadID + "</PayloadID>";
