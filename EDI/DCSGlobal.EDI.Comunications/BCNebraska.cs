...
{
    class BCNebraska : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "BCNEMCD";
...
//);
    ...
    (se, cert, chain, sslerror) =>
    {
        return true;
                ...
                {
                    log.ExceptionDetails("BCNEMCD sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("BCNEMCD Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                        ...
                        if (!string.IsNullOrEmpty(soapResult))
                        {
                            try
                            {
                                ...
                                if (xnError != null)
                                {
                                    ...
                                    if (_VMetricsLogging == 1)
                                    {
                                        log.ExceptionDetails("BCNEMCD Response ErrorCode ", errMsg);
                                    }
                                }
                                ...
                                if (payLoad != null)
                                {
                                    ...
                                    if (_VMetricsLogging == 1)
                                    {
                                        log.ExceptionDetails("BCNEMCD Response Payload  ", _RES);
                                    }
                                }
                            }
                            catch (Exception ex)
                            {
                                r = -2;
                                log.ExceptionDetails("BCNEMCD Reading Response Failed ", ex);
                            }
                        }
            ...
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("BCNEMCD GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " | EBR ID:" + _EBRID + " | Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
