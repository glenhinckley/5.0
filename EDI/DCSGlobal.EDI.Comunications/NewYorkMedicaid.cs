...
{
    class NewYorkMedicaid : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "NYMCD";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
               ...
               // ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls | SecurityProtocolType.Ssl3;
                ...
                {
                    log.ExceptionDetails("NYMCD sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                if (_VMetricsLogging == 1)
                {
                    log.ExceptionDetails("NYMCD Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("NYMCD Response ErrorCode ", Err);
                            }
                        }
                        if (Err != string.Empty && Err != "Success")
                        {
                            MLOG_ID = log.MLOG(_VendorName, Err, _EBRID, stime, rtime, B_RES, B_REQ, PayloadIDTX, PayloadIDRX, soapResult, _REQ);
                            log.ExceptionDetails(_VendorName + " ErrorMSG = '" + ErrorMsg + "'  req and res are in MLOG with MLOG_ID " + Convert.ToString(MLOG_ID), Convert.ToString(DateTime.Now) + " | PayloadID=" + sPayloadID + " | Error Status= " + ErrorMsg + " Response = " + soapResult + " | Vendor Name =  " + _VendorName + "| Payor Code=" + _PayorCode + "|" + "waitstate=" + Convert.ToString(m) + "|" + _wsUrl);
                            r = -2;
                        }
                        else
                        {
                            _RES = xd.DocumentElement.GetElementsByTagName("Payload")[0].InnerText;
                            sPayload = _RES;
                            r = 0;
                            if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("NYMCD Response Payload  ", _RES);
                            }
                        }
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("NYMCD    Reading Response Failed ", ex);
                    }
                }
            }
            ...
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("NYMCD GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = soap + Convert.ToString("<wsse:Password Type='wsse:PasswordText'>" + _Passwd + "</wsse:Password>");
            soap = soap + Convert.ToString("</wsse:UsernameToken>");
            soap = soap + Convert.ToString("</wsse:Security>");
            soap = soap + Convert.ToString("</soap:Header>");
            soap = soap + Convert.ToString("<soap:Body>");
            soap = soap + Convert.ToString("<cor:COREEnvelopeRealTimeRequest xmlns:cor='http://emedny.org/CORERule.xsd'>");
            soap = soap + Convert.ToString("<cor:PayloadType>X12_270_Request_005010X279A1</cor:PayloadType>");
            soap = soap + Convert.ToString("<cor:ProcessingMode>RealTime</cor:ProcessingMode>");
            soap = soap + Convert.ToString("<cor:PayloadID>" + _PayloadID + "</cor:PayloadID>");
            soap = soap + Convert.ToString("<cor:TimeStamp>" + DateTime.UtcNow.ToString("yyyy-MM-ddThh:mm:ssZ") + "</cor:TimeStamp>");
            soap = soap + Convert.ToString("<cor:SenderID>" + _UserName + "</cor:SenderID>");
            soap = soap + Convert.ToString("<cor:ReceiverID>" + _Passwd + "</cor:ReceiverID>");
            soap = soap + Convert.ToString("<cor:CORERuleVersion>2.2.0</cor:CORERuleVersion>");
            //soap = soap + Convert.ToString("<cor:Payload>ISA*00*          *00*          *ZZ*00JA           *ZZ*EMEDNYREL      *170922*1218*^*00501*895598105*0*P*:~GS*HS*00JA*EMEDNYREL      *20170922*1218*589486*X*005010X279A1~ST*270*589486*005010X279A1~BHT*0022*13*CFCFA89C8B73456*20170919*1218~HL*1**20*1~NM1*PR*2*New York Medicaid*****FI*141797357~HL*2*1*21*1~NM1*1P*2*RCHP*****XX*1235131442~HL*3*2*22*0~NM1*IL*1*HECK*DANIELLE*M***MI*FY96116R~DMG*D8*19850902~DTP*291*D8*20170919~EQ*30~SE*12*589486~GE*1*589486~IEA*1*895598105~</cor:Payload>");
            soap = soap + Convert.ToString("<cor:Payload>" + edi + "</cor:Payload>");
