...
{
    class FloridaDCSWebService : IDisposable
    {
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("FLMCD DCSWS sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.GetResponse())
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("FLMCD DCSWS Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        soapResult = soapResult.Replace("&lt;", "<").Replace("&gt;", ">");
            ...
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("FLMCD DCSWS GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = soap + Convert.ToString("<getVendorResponse xmlns='http://tempuri.org/'>");
            ...
            soap = soap + Convert.ToString("</getVendorResponse>");
            soap = soap + Convert.ToString("</soap:Body>");
