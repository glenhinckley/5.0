...
{
    class IndianaMedicaid : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "INMCD";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("Indiana sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("Indiana Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (xnError != null)
                        {
                            ...
                            {
                                log.ExceptionDetails("Indiana Response ErrorCode ", Err);
                            }
                        }
                        ...
                        { 
                            ...
                            _RES = xd.DocumentElement.GetElementsByTagName("Payload")[0].InnerText;
                             ...
                             if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("Indiana Response Payload  ", _RES);
                            }
                        }
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("Indiana Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("Indiana GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = soap + Convert.ToString("<soap:Envelope xmlns:cor='http://www.caqh.org/SOAP/WSDL/CORERule2.2.0.xsd' xmlns:soap='http://www.w3.org/2003/05/soap-envelope'>");
            soap = soap + Convert.ToString("<soap:Header xmlns:wsa='http://www.w3.org/2005/08/addressing'>");
            ...
            soap = soap + Convert.ToString("<wsa:Action>http://www.caqh.org/SOAP/WSDL/CORETransactions/RealTimeTransactionRequest</wsa:Action>");
            soap = soap + Convert.ToString("<wsa:To>https://coresvcmodel.xinnproad.xin.dcs-usps.com/HP.Core/CoreTransactions.svc</wsa:To>");
