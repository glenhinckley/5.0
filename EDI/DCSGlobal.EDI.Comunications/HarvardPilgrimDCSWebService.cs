...
{
    class HarvardPilgrimDCSWebService : IDisposable
    {
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    ...
                    Err = CleanResponse(soapResult, "<ErrorCode>", "</ErrorCode>");
                    ...
                    if (B_RES != B_REQ)
                    {
                        ...
                        MLOG_ID = log.MLOG(_VendorName, Err, _EBRID, stime, rtime, B_RES, B_REQ, PayloadIDTX, PayloadIDRX, sPayload, _REQ);
                        r = -4;
                        log.ExceptionDetails(_VendorName + " BHT03 mismatch req and res are in MLOG with MLOG_ID " + Convert.ToString(MLOG_ID), Convert.ToString(DateTime.Now) + " | PayloadID=" + sPayloadID + " | Error Status= " + ErrorMsg + " Response = " + soapResult + " | Vendor Name =  " + _VendorName + "| Payor Code=" + _PayorCode + "|" + "waitstate=" + Convert.ToString(m) + "|" + _wsUrl);
            ...
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("CENTENE GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
