...
{
    class SCMCDDCSWebService : IDisposable
    {
        ...
        private string _VendorName = "SCMCD";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("DCS SCMCD sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.GetResponse())
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("DCS SCMCD Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    ...
                    try
                    {
                        ...
                        //soapResult = soapResult.Replace("<![CDATA[", "").Replace("]]>", "");
                        ...
                        soapResult = CleanResponse(soapResult, "<return>", "</return>");
                        s1 = soapResult;
                        if (s1.StartsWith("ISA"))
                        {
                            _RES = s1;
                            sPayload = _RES;
                            if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("DCS SCMCD REcived  at " + Convert.ToString(rtime) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _RES);
                            }
                            r = 0;
                        }
                        else
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("DCS SCMCD Reading Response Failed ", ex);
                    }
                }
                else
                {
                    MLOG_ID = log.MLOG(_VendorName, "Soap Result Failed", _EBRID, stime, rtime, soapResult, B_REQ, PayloadIDTX, PayloadIDRX, _RES, _REQ);
                    log.ExceptionDetails(_VendorName + " Soap Result Failed   " + Convert.ToString(MLOG_ID), Convert.ToString(DateTime.Now) + " | PayloadID=" + sPayloadID + " | Error Status= " + ErrorMsg + " Response = " + soapResult + " | Vendor Name =  " + _VendorName + "| Payor Code=" + _PayorCode + "|" + "waitstate=" + Convert.ToString(m) + "|" + _wsUrl);
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("DCS SCMCD GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
