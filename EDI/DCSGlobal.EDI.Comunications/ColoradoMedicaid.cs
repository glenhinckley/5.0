...
{
    class ColoradoMedicaid : IDisposable
    {
        private logExecption log = new logExecption();
        private StringStuff ss = new StringStuff();

        private string _ConnectionString = string.Empty;
        private string _VendorName = "COLORADO";
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("ColoradoMedicaid sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                if (_VMetricsLogging == 1)
                {
                    log.ExceptionDetails("ColoradoMedicaid Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        _RES = XMLEDI271.GetElementsByTagName("a:Payload")[0].InnerText;
                        PayloadIDRX = XMLEDI271.GetElementsByTagName("a:PayloadID")[0].InnerText;
                        r = 0;
                        if (_RES.StartsWith("ISA"))
                        {
                            ...
                            if (_VMetricsLogging == 1)
                            {
                                log.ExceptionDetails("ColoradoMedicaid    REcived  at " + Convert.ToString(rtime) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _RES);
                            }
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("ColoradoMedicaid Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("ColoradoMedicaid GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = "";
            soap = soap + "<soap:Envelope xmlns:soap='http://www.w3.org/2003/05/soap-envelope' xmlns:tem='http://tempuri.org/' xmlns:cor='http://www.caqh.org/SOAP/WSDL/CORERule2.2.0.xsd'>";
            soap = soap + "<soap:Header>";
            soap = soap + "<wsse:Security xmlns:wsse='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd' xmlns:wsu='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd'>";
            soap = soap + "<wsse:UsernameToken wsu:Id='UsernameToken-78487AC9AAA3F96B18146076044685612'>";
            ...
            soap = soap + "<cor:PayloadType>X12_270_Request_005010X279A1</cor:PayloadType>";
            soap = soap + "<cor:ProcessingMode>RealTime</cor:ProcessingMode>";
            soap = soap + "<cor:PayloadID>" + Convert.ToString(sPayloadID) + "</cor:PayloadID>";
            soap = soap + "<cor:TimeStamp>" + DateTime.UtcNow.ToString("yyyy-MM-ddThh:mm:fffZ") + "</cor:TimeStamp>";
            soap = soap + "<cor:SenderID>" + _SenderID + "</cor:SenderID>";
            soap = soap + "<cor:ReceiverID>" + _ReceiverID + "</cor:ReceiverID>";
            soap = soap + "<cor:CORERuleVersion>2.2.0</cor:CORERuleVersion>";
            soap = soap + "<cor:Payload>" + edi + "</cor:Payload>";
            soap = soap + "</tem:Request>";
            soap = soap + "</tem:RealTimeTransaction>";
            soap = soap + "</soap:Body>";
