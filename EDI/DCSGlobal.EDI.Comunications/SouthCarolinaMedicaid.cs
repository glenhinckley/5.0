...
{
    class SouthCarolinaMedicaid : IDisposable
    {
        ...
        public int GoToVendor()
        {
            ...
            try
            {
                ...
                {
                    log.ExceptionDetails("SC MEDICAID sent at " + Convert.ToString(sTimeStamp) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _REQ);
                }
                ...
                using (WebResponse webResponse = webRequest.EndGetResponse(asyncResult))
                {
                    ...
                    if (_VMetricsLogging == 1)
                    {
                        log.ExceptionDetails("SC MEDICAID Response Recived  at " + Convert.ToString(DateTime.Now), soapResult);
                    }
                }
                ...
                if (!string.IsNullOrEmpty(soapResult))
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(httpData))
                        {
                            ...
                            s1 = XMLEDI271.InnerText.Replace("<![CDATA[", "").Replace("]]>", "");
                            if (s1.StartsWith("ISA"))
                            {
                                ...
                                if (_VMetricsLogging == 1)
                                {
                                    log.ExceptionDetails("SC MCD REcived  at " + Convert.ToString(rtime) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl, _RES);
                                }
                                r = 0;
                            }
                            else
                    ...
                    {
                        r = -3;
                        log.ExceptionDetails("SC MCD Reading Response Failed ", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                log.ExceptionDetails("SC MCD GetResponse Failed Total Elasped ticks" + Convert.ToString(_elapsedTicks) + " Service time out  | " + _ServiceTimeOut + " | " + _VendorName + "|" + _PayorCode + "|" + _wsUrl + "|" + _REQ, ex);
            }
            return r;
        }
        ...
        private HttpWebRequest CreateWebRequest_old(string url)
        {
            HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create(url);
            //webRequest.Headers.Add("SOAPAction", action);
        ...
        private XmlDocument CreateSoapEnvelope(string edi)
        {
            ...
            soap = "<soap:Envelope xmlns:soap='http://www.w3.org/2003/05/soap-envelope' xmlns:ser='http://service.clemson.edu/'>";
            soap = soap + "<soap:Header>";
            soap = soap + "<wsse:Security soap:mustUnderstand='1' xmlns:wsse='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd' xmlns:wsu='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd' >";
            soap = soap + "<wsse:UsernameToken wsu:Id='UsernameToken-600'>";
            ...
            soap = soap + "</soap:Header>";
            soap = soap + "<soap:Body>";
            soap = soap + "<ser:processRequest>";
            soap = soap + "<ediInput>" + edi + "</ediInput>";
            soap = soap + "</ser:processRequest>";
            soap = soap + "</soap:Body>";
